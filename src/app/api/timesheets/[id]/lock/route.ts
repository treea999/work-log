...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) { try { const { id } = await params; return Response.json(await service.lock(await requireActor(), id)); } catch (error) { return errorResponse(error); } }
